...
debugMode && console.log("CONSOLE_ENV", CALUNGA_ENV);
...
app.get("*splat", (_, res) => {
  if (CALUNGA_ENV.NODE_ENV === "development") {
    res.send(`
      <style>pre { margin-left: 20px; }</style>
      You're running in development mode! The UI is served by rsbuild dev server on port 3000: <a href="http://localhost:3000">http://localhost:3000</a><br /><br />
